...
_Noreturn void __pthread_exit(void *result)
{
	...
	/* SGX_HACK
	 * The kernel will write 0 to &self->detach_state. But we are facing
	 * synchronization issues in consistency of buffer so we are using this hack
	 */
...
static int start(void *p)
{
	...
	if (self->unblock_cancel)
		__syscall(SYS_rt_sigprocmask, SIG_UNBLOCK,
			SIGPT_SET, 0, _NSIG/8);
	/* SGX_HACK
	 * Because outside thread is created using clone we need to fs pointer
	 * accordingly for CLONE_SETTLS
	 */

	__set_thread_area(p);
	
	/* SGX_HACK
	 * Because the set_tid_address is used that of pthread_create of glibc
	 * we need to set it explicitly for CLONE_CHILD_SETTID
	 */

	self->tid = syscall(SYS_set_tid_address, &self->detach_state);
